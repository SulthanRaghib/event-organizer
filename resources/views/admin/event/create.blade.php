...
@section('content')
    ...
    <form action="{{ route('event.add-event') }}" method="POST">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="date" class="form-control" id="tanggal_dibuat" name="tanggal_dibuat" value="{{ date('Y-m-d') }}"
                readonly>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="date" class="form-control" id="tanggal_event" name="tanggal_event"
                placeholder="Masukkan Tanggal Event" value="{{ old('tanggal_event') }}">
        </div>
        @error('tanggal_event')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="tempat_event" name="tempat_event"
                placeholder="Masukkan Tempat Event" value="{{ old('tempat_event') }}">
        </div>
        @error('tempat_event')
            <div class="alert alert-danger">{{ $message }}</div>
        @enderror
        <div class="form-group">
            ...
            <textarea class="form-control" id="deskripsi_event" name="deskripsi_event" rows="3"
                placeholder="Masukkan Deskripsi">{{ old('deskripsi_event') }}</textarea>
        </div>
